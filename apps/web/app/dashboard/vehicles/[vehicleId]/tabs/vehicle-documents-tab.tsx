...
export function VehicleDocumentsTab({ vehicleId, organizationId, customerId }: VehicleDocumentsTabProps) {
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      const docsRes = await documentsAPI.list(organizationId, {
        vehicleId,
  ...
  useEffect(() => {
    loadData();
  }, [organizationId, vehicleId, effectiveCustomerId]);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await documentsAPI.remove(organizationId, documentToDelete.id);
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8 text-muted-foreground">{t('vehicles.tabs.emptyDocuments')}</div>
      ) : (
      ...
      <DocumentFormDialog
        ...
        onSuccess={() => { loadData(); handleFormClose(); }}
      />
