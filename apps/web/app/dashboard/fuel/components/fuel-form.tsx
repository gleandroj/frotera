...
import type { FuelLog, FuelType, CreateFuelLogPayload, UpdateFuelLogPayload } from "@/lib/frontend/api-client";
...
export function FuelForm({ initialValues, onSubmit, loading = false }: FuelFormProps) {
  ...
  const schema = z.object({
    vehicleId: z.string({ required_error: t('fuel.form.vehicleRequired') }).min(1, t('fuel.form.vehicleRequired')),
    date: z.string({ required_error: t('fuel.form.dateRequired') }).min(1, t('fuel.form.dateRequired')),
    odometer: z.number({ required_error: t('fuel.form.odometerRequired') }).positive(t('fuel.form.odometerPositive')),
    liters: z.number({ required_error: t('fuel.form.litersRequired') }).positive(t('fuel.form.litersPositive')),
    pricePerLiter: z.number({ required_error: t('fuel.form.priceRequired') }).positive(t('fuel.form.pricePositive')),
    fuelType: z.enum(['GASOLINE', 'ETHANOL', 'DIESEL', 'ELECTRIC', 'GNV'], { required_error: t('fuel.form.fuelTypeRequired') }),
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        <FormItem>
          <FormLabel>{t('fuel.fields.totalCost')}</FormLabel>
